...
#2 Find avg by type
def calculate_average_rating_by_type():
  ...
  for rates_by_type in rating_types.keys():
      ratings = rating_types[rates_by_type]
      average_rating_by_type = sum(ratings) / len(ratings) if ratings else 0

      print(f"  {rates_by_type}: {average_rating_by_type}")


print(f" average rating out of 5 is {calculate_average_rating()}")
print(f" averate rating by type is ")
calculate_average_rating_by_type()
